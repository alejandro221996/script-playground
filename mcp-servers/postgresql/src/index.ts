...
class PostgreSQLMCPServer {
      ...
      try {
        switch (name) {
          ...
          case "list_mock_configs":
            return await this.listMockConfigs(
              (args.enabled_only as boolean) || false
            );
          ...
          case "search_mocks":
            return await this.searchMocks(args.search_term as string);
  ...
  private async listMockConfigs(enabledOnly: boolean = false): Promise<any> {
    let query = `
      SELECT 
        id,
        name,
  ...
  private async getMockStats(): Promise<any> {
    const statsQuery = `
      SELECT 
  ...
  private async searchMocks(searchTerm: string): Promise<any> {
    const query = `
      SELECT 
        id,
        name,
