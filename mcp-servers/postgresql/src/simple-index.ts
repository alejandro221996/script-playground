...
class SimpleMCPServer {
      ...
      try {
        switch (name) {
          case 'get_mock_context':
            return await this.getMockContext(args?.filter as string);
          
          case 'analyze_endpoint':
            return await this.analyzeEndpoint(args?.url as string, args?.method as string);
  ...
  private async getMockContext(filter?: string) {
    let query = `
      SELECT 
...
- Enable/disable mocks based on testing needs`,
  ...
  private async analyzeEndpoint(url: string, method?: string) {
    try {
...
${this.getEndpointRecommendations(hostname, pathname, similar.rows)}`,
  ...
  private getEndpointRecommendations(hostname: string, pathname: string, similar: any[]): string {
...
${recentActivity.rows.map(row => 
  `- **${row.date}**: ${row.mocks_created} mocks created`
).join('\n')}

### Popular Tags
${popularTags.rows.map(row => 
  `- **${row.tag}**: Used in ${row.usage_count} mocks`
).join('\n')}
...
- **Most Used Method**: ${Object.entries(stats.byMethod).sort(([,a], [,b]) => (b as number) - (a as number))[0]?.[0] || 'None'}
...
${this.getOptimizationSuggestions(stats)}`,
  ...
  private getOptimizationSuggestions(stats: any): string {
    const suggestions = [];
